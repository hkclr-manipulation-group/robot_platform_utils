...
constexpr int kTelemetryPort = 30201;
    ...
    bool readOnly)
{
    ...
    if (configRes.payload.status != CommandResponseStatus::kSuccess) {
        throw std::runtime_error("Config rejected for client 10001");
    }
}
    ...
    uint32_t sequenceId)
{
    ...
    if (commandRes.payload.status != CommandResponseStatus::kSuccess) {
        throw std::runtime_error("Command rejected for client 10001");
    }
}
...
int main()
{
    ...
    try {
        ...
        std::cout << "Client 10001 connected to server " << kServerIp << ":" << kCoreRequestPort
